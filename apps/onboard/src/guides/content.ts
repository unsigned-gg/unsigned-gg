...
export const GUIDES: Guide[] = [
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "Tailnet: an invite to the Tailscale tailnet (`tail769bd2.ts.net`), device approval, and an ACL grant for `tag:bastion` (SSH) and `tag:k8s` (admin UIs).",
    ...
    sections: [
      ...
      { kind: "h2", text: "The three doors" },
      {
        ...
        rows: [
          ...
          ["Admin UIs (*.tail769bd2.ts.net) + bastion SSH", "Tailnet ACL", "Approved device + tag grant"],
      ...
      { kind: "h2", text: "Tailnet-gated" },
      {
        ...
        items: [
          ...
          "Grafana `grafana.tail769bd2.ts.net`, Prometheus `prometheus.tail769bd2.ts.net`, Alertmanager `alertmanager.tail769bd2.ts.net`, Loki `loki.tail769bd2.ts.net` (API; explore via Grafana), OpenBao admin `openbao.tail769bd2.ts.net` — deliberately NOT on the public wildcard.",
        ],
      },
  ...
  // ── g. platform-map ───────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      { kind: "h2", text: "Tailnet-only — *.tail769bd2.ts.net" },
      {
        kind: "table",
        headers: ["service", "url", "auth", "purpose"],
        rows: [
          ["Grafana", "https://grafana.tail769bd2.ts.net", "tailnet + SSO", "Dashboards; Loki via Explore"],
          ["Prometheus", "https://prometheus.tail769bd2.ts.net", "tailnet", "Metrics"],
          ["Alertmanager", "https://alertmanager.tail769bd2.ts.net", "tailnet", "Alert routing"],
          ["Loki", "https://loki.tail769bd2.ts.net", "tailnet", "Log API (use Grafana to explore)"],
          ["OpenBao (admin)", "https://openbao.tail769bd2.ts.net", "tailnet + token", "OpenBao admin surface"],
